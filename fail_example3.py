...
for sd in [19]:
    ...
    for k in range(num_mc):
        ...
        print(f'theta exact / theta found: {theta / thetas[k]}\n')
...
print(f'pi/2 - Ave theta estimated: {0.5-np.mean(thetas)/np.pi}')
...
signal_exact = ula_signal.update_signal_signs(ula_signal.signs_exact)

plt.plot(np.real(signal_exact))
plt.plot(np.imag(signal_exact))
plt.show()

ula_signal_exact = ula_signal.get_ula_signal(signal_exact)
signal_found = ula_signal.update_signal_signs(signs_found)
ula_signal_found = ula_signal.get_ula_signal(signal_found)

plt.plot(np.real(signal_found))
plt.plot(np.imag(signal_found))
plt.show()
...
plt.show()
plt.plot(np.imag(ula_signal_found))
plt.plot(np.imag(ula_signal_exact))
# plt.plot(np.real(ula_signal_bad))
plt.show()
print(f'L2 Diff: {np.linalg.norm(ula_signal_exact-ula_signal_found,2)}')

print(np.real(ula_signal_found[0]))
print(np.real(ula_signal_exact[0]))
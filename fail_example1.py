...
for k in range(num_mc):
    ...
    signal = ula_signal.estimate_signal(n_samples=ula_signal.n_samples, theta=theta, eta=eta)
    print(f'ula_signal.signal[0] = {np.real(signal[0])}')
    objective = -np.inf
...
zero_signal = ula_signal.signal[0]

plt.plot(np.real(ula_signal.signal))
plt.plot(np.imag(ula_signal.signal))
plt.show()

print(f'Zero Query Estimate: {np.real(zero_signal)}')
print(ula_signal.depths)
print(ula_signal.n_samples)
